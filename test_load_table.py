...
def assert_non_unique_indexes(engine, table_name, *columns):
...
class TestLoadTable():
    ...
    def test_good_csv(self, tmpdir):
        ...
        assert_non_unique_indexes(self.engine, 'people',
                                  'name', 'dob', 'number_of_pets', 'weight')

        connection = self.engine.connect()
        results = connection.execute(
            select([people]).order_by(people.c.id)).fetchall()
        assert (0, 'Tom', '1980-02-26', 0, 146.8) == results[0]
        assert (1, 'Dick', '1982-03-14', 3, 183.1) == results[1]
        assert (2, 'Harry', '1972-11-24', 2, 178.7) == results[2]
